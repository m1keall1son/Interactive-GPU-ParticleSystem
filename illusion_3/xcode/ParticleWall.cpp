...
void Particlewall::setup(){
    ...
    img = Surface32f(VERTICES_X, VERTICES_Y, false);
    ...
    defaultFormat.enableDepthBuffer(false);
...
void Particlewall::update(){
    ...
    if(kinect.getTex()){
        gl::clear();
        gl::draw( kinectImage,  Rectf(Vec2f(0,0),Vec2f(800+mWTFoffset,800+mWTFoffset)) );
    }
  ...
  //  kinect.getTex().bind(1);
   ...
   // kinect.getTex().unbind();
    ...
    gl::setMatricesWindow( rect.getSize(), false ); // false to prevent vertical flipping
...
void Particlewall::draw(){
    ...
    if(!debug){
        ...
        pingpong.bind(0);
        gl::Texture & k = kinectFbo.getTexture();
        k.bind(1);
        ...
        drawShader->uniform("displacement", 1);
        drawShader->uniform("vid", 2);
