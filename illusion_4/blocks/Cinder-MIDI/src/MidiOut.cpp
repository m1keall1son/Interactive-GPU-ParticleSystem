...
bool MidiOut::sVerboseLogging = true;
...
std::vector<std::string> MidiOut::getPortList() const
{
	...
	for(unsigned int i = 0; i < mRtMidiOut->getPortCount(); ++i)
	{
        ...
        std::cout << i << ": " <<  mRtMidiOut->getPortName(i).c_str() << std::endl;
	}
	return portList;
}
...
{
	sendMessage(MIDI_CHANNEL_PRESSURE+channel-1, value);
}
void MidiOut::sendPolyAftertouch(int channel, int pitch, int value)
{
	sendMessage(MIDI_POLY_PRESSURE+channel-1, pitch, value);
}
